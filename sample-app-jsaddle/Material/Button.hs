...
module Material.Button
    ...
    , setTouch
    , setAttributes
    , Material.Button.text, outlined, raised, unelevated
    ) where
...
touchElt (Config { touch=touch }) =
    if touch then
        Just (div_ [ class_ "mdc-button__touch" ] [])
