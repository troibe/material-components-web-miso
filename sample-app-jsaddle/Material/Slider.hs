...
discreteCs (Config { discrete=discrete }) =
    if discrete then
        Just (Miso.class_ "mdc-slider--discrete")
...
displayMarkersCs (Config { discrete=discrete, displayMarkers=displayMarkers }) =
    if discrete && displayMarkers then
        Just (Miso.class_ "mdc-slider--display-markers")
...
thumbContainerElt (Config { discrete=discrete }) =
    Miso.div_ [ Miso.class_ "mdc-slider__thumb-container" ]
        (if discrete then
