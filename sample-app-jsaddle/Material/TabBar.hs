    ...
    let
        activeTabIndex =
            indexedMap pair tabs
                ...
                |> fmap fst
    in
    fmap (Miso.intProp "activeTabIndex") activeTabIndex
...
tabStackedCs (Config { stacked=stacked }) =
    if stacked then
        Just (Miso.class_ "mdc-tab--stacked")
...
tabMinWidthCs (Config { minWidth=minWidth }) =
    if minWidth then
        Just (Miso.class_ "mdc-tab--min-width")
...
tabContentElt barConfig@Config { indicatorSpansContent=indicatorSpansContent } config_ content =
    Just
        (Miso.div_ [ Miso.class_ "mdc-tab__content" ]
            (if indicatorSpansContent then
                Maybe.mapMaybe id
                    [ tabIconElt content
