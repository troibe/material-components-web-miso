...
listItemView (config_@Config { additionalAttributes, href }) nodes =
    (\attributes ->
        if href /= Nothing then
            Miso.nodeHtml "mdc-list-item" [] [ Miso.a_ attributes nodes ]

        else
            Miso.nodeHtml "mdc-list-item" attributes nodes
    )
        (Maybe.mapMaybe id
            [ listItemCs
            , hrefAttr config_
...
disabledCs (Config { disabled }) =
    if disabled then
        Just (Miso.class_ "mdc-list-item--disabled")
...
selectedCs (Config { selection }) =
    if selection == Just Selected then
        Just (Miso.class_ "mdc-list-item--selected")
...
activatedCs (Config { selection }) =
    if selection == Just Activated then
        Just (Miso.class_ "mdc-list-item--activated")
...
ariaSelectedAttr (Config { selection }) =
    if selection /= Nothing then
        Just (Miso.stringProp "aria-selected" "true")
...
text additionalAttributes (TwoLineListItemText { primary, secondary }) =
    Miso.div_ (Miso.class_ "mdc-list-item__text" : additionalAttributes)
        [ primaryText [] primary
