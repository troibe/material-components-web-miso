...
nonInteractiveCs (Config { nonInteractive }) =
    if nonInteractive then
        Just (Miso.class_ "mdc-list--non-interactive")
...
denseCs (Config { dense }) =
    if dense then
        Just (Miso.class_ "mdc-list--dense")
...
avatarListCs (Config { avatarList }) =
    if avatarList then
        Just (Miso.class_ "mdc-list--avatar-list")
...
twoLineCs (Config { twoLine }) =
    if twoLine then
        Just (Miso.class_ "mdc-list--two-line")
...
selectedIndexProp listItems =
    ...
    in
      Just (Miso.intProp "selectedIndex" selectedIndex)
