...
leadingCs message_ =
    Maybe.maybe
        ...
        (\(Message { leading=leading }) ->
            if leading then
                Just (Miso.class_ "mdc-snackbar--leading")
...
stackedCs message_ =
    Maybe.maybe
        ...
        (\(Message { stacked=stacked }) ->
            if stacked then
                Just (Miso.class_ "mdc-snackbar--stacked")
...
timeoutMsProp message_ =
    let
        ...
        indefiniteTimeout =
            -1
    in
    Just (Miso.intProp "timeoutMs" normalizedTimeoutMs)
...
surfaceElt messageId message_ =
    Miso.div_ [ Miso.class_ "mdc-snackbar__surface" ]
        [ labelElt message_
...
labelElt (Message { label=label }) =
    Miso.div_ [ Miso.class_ "mdc-snackbar__label", ariaStatusRoleAttr, ariaPoliteLiveAttr ]
        [ Miso.text (Miso.String.toMisoString label) ]


actionsElt :: MessageId -> Message msg -> Miso.View msg
actionsElt messageId message_ =
    Miso.div_ [ Miso.class_ "mdc-snackbar__actions" ]
        (Maybe.mapMaybe id
