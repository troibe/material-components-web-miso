...
chip (Internal.Chip (config_@Internal.Config { Internal.additionalAttributes=additionalAttributes }) label) =
    Miso.div_ [ Miso.class_ "mdc-touch-target-wrapper" ]
        [ Miso.nodeHtml "mdc-chip"
            (Maybe.mapMaybe id
                [ chipCs
                , chipTouchCs
                , rowRole
...
checkmarkElt =
    Just
        (Miso.div_ [ Miso.class_ "mdc-chip__checkmark" ]
            [ Svg.svg_
...
primaryActionElt label =
    Just <|
        Miso.span_ [ chipPrimaryActionCs, gridcellRole ]
