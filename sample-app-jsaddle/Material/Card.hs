        ...
        Just (Actions { buttons=buttons, icons=icons, fullBleed=fullBleed }) ->
            [ div_
                (mapMaybe id
                    ...
                    , if fullBleed then
                        Just (class_ "mdc-card__actions--full-bleed")
                ...
                (List.concat
                    [ if not (List.null buttons) then
                        [ div_ [ class_ "mdc-card__action-buttons" ]
                            (List.map (\(Button button_) -> button_) buttons)
                        ]

                      else
                        []
                    , if not (List.null icons) then
                        [ div_ [ class_ "mdc-card__action-icons" ]
