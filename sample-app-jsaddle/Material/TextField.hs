...
outlinedCs outlined_ =
    if outlined_ then
        Just (Miso.class_ "mdc-text-field--outlined")
...
fullwidthCs (Config { fullwidth=fullwidth }) =
    if fullwidth then
        Just (Miso.class_ "mdc-text-field--fullwidth")
...
disabledCs (Config { disabled=disabled }) =
    if disabled then
        Just (Miso.class_ "mdc-text-field--disabled")
...
ariaLabelAttr (Config { fullwidth=fullwidth, placeholder=placeholder, label=label }) =
    if fullwidth then
        Maybe.maybe Nothing (\x->Just(Miso.textProp "aria-label" (Miso.String.toMisoString x))) label
        ...
        Just str ->
            Miso.div_
                [ if Maybe.fromMaybe "" value /= "" then
                    Miso.class_ (Miso.String.toMisoString (floatingLabelCs ++ " " ++ floatingLabelFloatAboveCs))

                  else
                    Miso.class_ (Miso.String.toMisoString floatingLabelCs)
                , Miso.textProp "foucClassNames"
...
noLabelCs (Config { label=label }) =
    if label == Nothing then
        Just (Miso.class_ "mdc-text-field--no-label")
...
notchedOutlineElt config_ =
    Miso.div_ [ Miso.class_ "mdc-notched-outline" ]
